...
const seedSites = [
  { id: 's1', name: 'ABC Residency', orderId: 'ORD-1024', customer: 'Anil Kulkarni', contact: '99887 12001', address: 'Baner Road, Pune', city: 'Pune', installerId: 'i1', assignedAt: '12 Sep 2026', status: 'COMPLETED', expectedVisits: 2, doors: [{ type: 'Single Leaf Dead Lock', quantity: 8, charge: 1200 }, { type: 'Double Leaf Dead Lock', quantity: 4, charge: 1800 }], visitCharge: 500, orderFile: 'order_1024.pdf', paymentStatus: 'PAYMENT_PENDING' },
  { id: 's2', name: 'Pune Site 104', orderId: 'ORD-1041', customer: 'Riya Developers', contact: '99887 12002', address: 'Wakad, Pune', city: 'Pune', installerId: 'i2', assignedAt: '15 Sep 2026', status: 'COMPLETED', expectedVisits: 3, doors: [{ type: 'Glass Door', quantity: 3, charge: 2550 }, { type: 'Single Leaf Panic Bar', quantity: 6, charge: 1400 }], visitCharge: 500, orderFile: 'pune_104.xlsx', paymentStatus: 'PAYMENT_PENDING' },
  ...
  { id: 's4', name: 'Nashik Heights', orderId: 'ORD-1096', customer: 'Kale Homes', contact: '99887 12004', address: 'College Road, Nashik', city: 'Nashik', installerId: 'i4', assignedAt: '06 Sep 2026', status: 'ASSIGNED', expectedVisits: 2, doors: [{ type: 'Double Leaf Dead Lock', quantity: 5, charge: 1700 }], visitCharge: 450, orderFile: null, paymentStatus: 'NOT_READY' },
...
export function AdminDataProvider({ children }) {
  ...
  const value = useMemo(() => ({
    installers, sites, visits, cities, doorTypes,
    addCity: (name) => {
    ...
    saveSite: (site) => {
      ...
      const savedSite = snapshot.id ? snapshot : { ...snapshot, id: `s${Date.now()}`, assignedAt: '22 Sep 2026', paymentStatus: 'NOT_READY' };
    ...
    updateVisit: (id, status) => {
      ...
      const site = sites.find((item) => item.id === visit.siteId);

      if (!site || site.status !== 'COMPLETED') return;

      const hasPendingExtraVisit = visits.some(
        (item) =>
          item.siteId === visit.siteId &&
          item.type === 'EXTRA' &&
          item.id !== visit.id &&
      ...
      if (
        ...
        !hasPendingExtraVisit
      ) {
        setSites((x) =>
          x.map((item) =>
            item.id === visit.siteId
              ? {
                ...item,
    ...
    }, markPaid: (siteId) => setSites((x) => x.map((item) => item.id === siteId ? { ...item, paymentStatus: 'PAID', status: 'PAID' } : item))
  }), [installers, sites, visits, cities]);
