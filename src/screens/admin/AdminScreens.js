...
const InstallerRow = ({ installer, navigation }) => { const { sites } = useAdminData(); const assigned = sites.filter((x) => x.installerId === installer.id); return <Pressable style={s.card} onPress={() => navigation.navigate('InstallerDetails', { installerId: installer.id })}><View style={s.row}><Text style={s.cardTitle}>{installer.name}</Text><StatusBadge status={installer.status} /></View><Text style={s.meta}>{installer.phone}  •  {installer.city}</Text><Text style={s.meta}>Assigned: {assigned.length}  •  Active: {assigned.filter((x) => x.status === 'IN_PROGRESS').length}</Text></Pressable>; };
...
export function CitiesScreen({ navigation }) { const { cities, installers, sites } = useAdminData(); return <Page><View style={s.row}><View><Text style={s.title}>Cities</Text><Text style={s.subtitle}>View installation operations by location.</Text></View><Pressable onPress={() => navigation.navigate('AddCity')}><Text style={styles.add}>+ Add City</Text></Pressable></View>{cities.map((city) => { const citySites = sites.filter((x) => x.city === city); return <Pressable key={city} style={s.card} onPress={() => navigation.navigate('CityDetails', { city })}><Text style={s.cardTitle}>{city}</Text><Text style={s.meta}>Installers: {installers.filter((x) => x.city === city).length}  •  Active Sites: {citySites.filter((x) => x.status === 'IN_PROGRESS').length}</Text><Text style={s.meta}>Completed: {citySites.filter((x) => x.status === 'COMPLETED').length}</Text></Pressable>; })}</Page>; }
...
export function SitesScreen({ navigation }) { const { sites,cities,installers }=useAdminData();const [query,setQuery]=useState(''),[city,setCity]=useState('All'),[status,setStatus]=useState('All');const list=sites.filter((x)=>(city==='All'||x.city===city)&&(status==='All'||x.status===status)&&`${x.name} ${x.orderId}`.toLowerCase().includes(query.toLowerCase()));return <Page><View style={s.row}><View><Text style={s.title}>Sites / Jobs</Text><Text style={s.subtitle}>Manage assigned installation work.</Text></View><Pressable onPress={()=>navigation.navigate('AssignSite')}><Text style={styles.add}>+ Assign</Text></Pressable></View><SearchBar value={query} onChangeText={setQuery} placeholder="Search site or order ID"/><FilterChips options={['All',...cities]} selected={city} onSelect={setCity}/><FilterChips options={['All','ASSIGNED','IN_PROGRESS','COMPLETED','PAID']} selected={status} onSelect={setStatus}/>{list.length?list.map((x)=><SiteRow key={x.id} site={x} navigation={navigation}/>):<EmptyState text="No sites found for these filters."/>}</Page>; }
